...
{
    public class EfGetUApplicationsQuery : EfUseCase, IGetApplicationsQuery
    {
        public EfGetUApplicationsQuery(AspContext context) : base(context)
        {
        }
        ...
        public PagedResponse<ApplicationDto> Execute(int id)
        {
            ...
            {
             Data = query.Where(x => x.IsActive && x.UserId == id).Select(x => new ApplicationDto
                {
                    Id = x.Id,
                    Letter = x.Letter,
                    Status = x.Status,
